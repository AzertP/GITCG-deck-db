...
const getCharacterCards = async () => {
    ...
    const response = await axios.get<CharacterCard[]>(`${API_URL}/card/characters`)
    return response.data
}

const getActionCards = async () => {
    // console.log(`${API_URL}/actions`)
    const response = await axios.get<ActionCard[]>(`${API_URL}/card/actions`)
    return response.data
}

const getCardById = async (id: number) => {
    const response = await axios.get<CardStats>(`${API_URL}/card/${id}`)
    return response.data
}
